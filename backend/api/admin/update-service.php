...
if (!isset($data->id) || !isset($data->name) || !isset($data->category) || !isset($data->description)) {
...
try {
    ...
    $stmt = $pdo->prepare("UPDATE services SET name = ?, category = ?, description = ?, icon = ? WHERE id = ?");
    $stmt->execute([$data->name, $data->category, $data->description, $data->icon ?? null, $data->id]);
